...
import './reservecars.css'
import {useNavigate} from 'react-router-dom'

const ReserveCars = ({setOpen, CarId}) => {

  const navigate = useNavigate();

  const [selectedSeat, setSelectedSeat] = useState([]);
  const {data, loading, error} = useFetch(`/api/cars/cars/${CarId}`);
    ...
    try {
      ...
      await Promise.all(
        selectedSeat.map((seatId) => {
          const res = axios.put(`/api/cars/availability/${seatId}`, {
            dates: allDates,
          });
          return res.data;
        })
